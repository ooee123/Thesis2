...
int tcpServerSetup(int a) /* int tcpServerSetup ( int portNum ) { */{
...
if (a) /* if (portNum) */ {
c.sin_port = htons(a); /* local . sin_port = htons ( portNum ) ; */
} else {
...
memcpy((char *)&e.sin_addr, (char *)d->h_addr_list[0], d->h_length); /* memcpy ( ( char * ) & remote . sin_addr , ( char * ) hp -> h_addr_list [ 0 ] , hp -> h_length ) ; */
e.sin_port = htons(atoi(b)); /* remote . sin_port = htons ( atoi ( port ) ) ; */
if (connect(c, (struct sockaddr *)&e, sizeof(struct sockaddr_in))<0) /* if (connect(socket_num, (struct sockaddr *)&remote, sizeof(struct sockaddr_in))<0) */ {
