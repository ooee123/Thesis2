...
int tcpServerSetup(short portNum) {
	int server_socket = 0;
...
int readSocket(int socket, char buf[]) {
    long bytesRead;
    long accumulatedBytesRead = 0;
    int length = 0;
    if ((bytesRead = recv(socket, buf, HEADER_SIZE, 0)) < 0) {
        perror("Error reading");
        return -1;
    } else {
        accumulatedBytesRead += bytesRead;
    }
    if (accumulatedBytesRead >= 2) {
        length = ntohs(*(n_short *)buf);
    }
    while (accumulatedBytesRead < length) {
        if ((bytesRead = recv(socket, buf + accumulatedBytesRead,
            length - accumulatedBytesRead, 0)) < 0) {
            perror("Error reading");
            return -1;
        } else {
            accumulatedBytesRead += bytesRead;
    ...
    return (int)accumulatedBytesRead;
}
