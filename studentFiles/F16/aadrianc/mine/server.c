...
int b = 0; /* int foundFD = 0 ; */
int c = 0; /* int i = 0 ; */
int *d; /* int * newClientFDs ; */
char **e; /* char * * newHandles ; */
global_c--; /* clientNum -- ; */
d = (int *)malloc(sizeof(int) * global_c); /* newClientFDs = ( int * ) malloc ( sizeof ( int ) * clientNum ) ; */
e = (char **)malloc(sizeof(char *) * global_c); /* newHandles = ( char * * ) malloc ( sizeof ( char * ) * clientNum ) ; */
for (c = 0; c<=global_c; c++) {
if (global_d[c] != a) /* if (clientFDs[i] != fd) */ {
if (!b) /* if (!foundFD) */ {
d[c] = global_d[c]; /* newClientFDs [ i ] = clientFDs [ i ] ; */
e[c] = global_a[c]; /* newHandles [ i ] = handles [ i ] ; */
} else {
d[c - 1] = global_d[c]; /* newClientFDs [ i - 1 ] = clientFDs [ i ] ; */
e[c - 1] = global_a[c]; /* newHandles [ i - 1 ] = handles [ i ] ; */
}
} else {
b = 1; /* foundFD = 1 ; */
if (global_a[c] != ((void *)0 )) /* if (handles[i] != ((void *)0 )) */ {
printf("Bye %s\n", global_a[c]); /* printf ( "Bye %s\n" , handles [ i ] ) ; */
}
free(global_a[c]); /* free ( handles [ i ] ) ; */
close(global_d[c]); /* close ( clientFDs [ i ] ) ; */
}
}
free(global_a); /* free ( handles ) ; */
free(global_d); /* free ( clientFDs ) ; */
global_a = e; /* handles = newHandles ; */
global_d = d; /* clientFDs = newClientFDs ; */
}
...
FD_SET(global_b, &b); /* FD_SET ( serverFD , & fdvar ) ; */
if (global_c>0) /* if (clientNum>0) */ {
c = global_d[global_c - 1] + 1; /* maxFD = clientFDs [ clientNum - 1 ] + 1 ; */
}
for (a = 0; a<global_c; a++) {
FD_SET(global_d[a], &b); /* FD_SET ( clientFDs [ i ] , & fdvar ) ; */
}
if (mySelect(c, (fd_set *)&b, ((void *)0 ), ((void *)0 ), ((void *)0 )) == -1) /* if (mySelect(maxFD, (fd_set *)&fdvar, ((void *)0 ), ((void *)0 ), ((void *)0 )) == -1) */ {
perror("Error selecting FD to read from"); /* perror ( "Error selecting FD to read from" ) ; */
exit(1); /* exit ( 1 ) ; */
}
if (FD_ISSET(global_b, &b)) /* if (FD_ISSET(serverFD, &fdvar)) */ {
return global_b; /* return serverFD */
} else {
for (a = 0; a<global_c; a++) {
if (FD_ISSET(global_d[a], &b)) /* if (FD_ISSET(clientFDs[i], &fdvar)) */ {
return global_d[a]; /* return clientFDs[i] */
}
}
}
return -1; /* return -1 */
...
char *c = ((void *)0 ); /* char * dest = ( ( void * ) 0 ) ; */
int d = 0; /* int sent = 0 ; */
int e = 0; /* int i = 0 ; */
getDestHandle(b, &c); /* getDestHandle ( packet , & dest ) ; */
for (e = 0; e<global_c; e++) {
if (strcmp(c, global_a[e]) == 0) /* if (strcmp(dest, handles[i]) == 0) */ {
d = 1; /* sent = 1 ; */
if (sendPacket(b, global_d[e])<0) /* if (sendPacket(packet, clientFDs[i])<0) */ {
perror("Packet Broadcast"); /* perror ( "Packet Broadcast" ) ; */
exit(1); /* exit ( 1 ) ; */
}
break;
