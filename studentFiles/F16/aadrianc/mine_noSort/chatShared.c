...
int test() /* int test ( ) { */{
char *a = "abc"; /* char * h1 = "abc" ; */
char *b = "xyz"; /* char * h2 = "xyz" ; */
char *c = "Hello world"; /* char * mssg = "Hello world" ; */
char *d; /* char * handle1 ; */
char *e; /* char * handle2 ; */
char *f; /* char * myMssg ; */
packetHandle *g = makePacketMssg(1, getHLen(a), a, getHLen(b), b, c); /* packetHandle * hPack = makePacketMssg ( 1 , getHLen ( h1 ) , h1 , getHLen ( h2 ) , h2 , mssg ) ; */
printf("%d\n", headerGetLen(&(g->head ))); /* printf ( "%d\n" , headerGetLen ( & ( hPack -> head ) ) ) ; */
getDestHandle(g, &d); /* getDestHandle ( hPack , & handle1 ) ; */
getSrcHandle(g, &e); /* getSrcHandle ( hPack , & handle2 ) ; */
getMssg(g, &c); /* getMssg ( hPack , & mssg ) ; */
printf("%s\n", d); /* printf ( "%s\n" , handle1 ) ; */
printf("%s\n", e); /* printf ( "%s\n" , handle2 ) ; */
printf("%s\n", c); /* printf ( "%s\n" , mssg ) ; */
return 0; /* return 0 */
...
packetHandle *c = (packetHandle *)a; /* packetHandle * hPack = ( packetHandle * ) packet ; */
unsigned d = c->handleLen; /* unsigned destLen = hPack -> handleLen ; */
unsigned e = headerGetLen(a); /* unsigned packetLen = headerGetLen ( packet ) ; */
unsigned f; /* unsigned srcLen ; */
c = ((void *)c ) + c->handleLen + sizeof(packetHandle) - 1 - sizeof(header); /* hPack = ( ( void * ) hPack ) + hPack -> handleLen + sizeof ( packetHandle ) - 1 - sizeof ( header ) ; */
f = c->handleLen; /* srcLen = hPack -> handleLen ; */
unsigned g = e - (d + 1 ) - (f + 1 ); /* unsigned mssgLen = packetLen - ( destLen + 1 ) - ( srcLen + 1 ) ; */
*b = malloc(g); /* * mssg = malloc ( mssgLen ) ; */
memcpy(*b, &(c->handleStart ) + c->handleLen, g); /* memcpy ( * mssg , & ( hPack -> handleStart ) + hPack -> handleLen , mssgLen ) ; */
}
